...
## Пользователи команды (tldw1–tldw4)

Кроме админа (`WEB_LOGIN`) поддерживаются до четырёх пользователей с фиксированными
логинами `tldw1`..`tldw4`. У каждого — **свой вход и своя изолированная история**
обработок (задача видна только своему владельцу).
...
```
Пустой пароль → пользователь не создаётся. После правки `.env` пересоздать `.htpasswd`:
```bash
docker compose up -d --build nginx
```
Как это работает: nginx проверяет basic-auth и пробрасывает логин в web заголовком
`X-Auth-User`; web пишет его в колонку `owner` и фильтрует историю по нему. Заголовок
подделать нельзя — порт web виден только внутри docker-сети.

> ⚠️ История, созданная **до** появления пользователей (колонка `owner` пустая), видна
> только при входе без пользователя (dev). Для рабочих аккаунтов история начинается с нуля.

## Логи

Docker-логи ротируются автоматически (в `docker-compose.yml`: `json-file`, 10 МБ × 3 файла
на контейнер) — забить диск они не могут. Cron-лог обновления yt-dlp
(`/var/log/tldw-ytdlp.log`) растёт медленно; при желании подключите `logrotate`.

## Загрузка по ссылке (yt-dlp)

> Полный план и варианты диагностики/починки VK и YouTube — в [VIDEO_LINKS.md](VIDEO_LINKS.md).
